...
const IntelPanel = ({country, onClose}) => {
  ...
  const exportText=`SIGNAL — COUNTRY INTELLIGENCE REPORT
...
Generated by SIGNAL — Democratic Risk Intelligence (AlgoViva)`;
  ...
  const handleExport=()=>{
    ...
    a.href=url; a.download=`SIGNAL_${country.name.replace(/\s+/g,"_")}_${new Date().toISOString().slice(0,10)}.txt`;
  ...
  return(
    <div style={{background:"#0d0d1a",border:`1px solid ${col}33`,borderTop:`3px solid ${col}`,borderRadius:8,display:"flex",flexDirection:"column",height:"100%",animation:"fadeIn 0.2s ease",overflow:"hidden"}}>
      ...
      <div style={{padding:"12px 14px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.02)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          <div>
            ...
            <div style={{fontSize:8.5,color:"rgba(255,255,255,0.35)",fontFamily:"'IBM Plex Mono',monospace",marginTop:2,letterSpacing:"0.06em"}}>{country.region.toUpperCase()} · INTELLIGENCE BRIEF</div>
          </div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
            ...
            <button onClick={onClose} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",color:"rgba(255,255,255,0.5)",cursor:"pointer",borderRadius:4,width:22,height:22,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,marginTop:4}}>✕</button>
          </div>
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
          ...
          <div>
            ...
            <div style={{fontSize:7.5,color:"rgba(255,255,255,0.25)",fontFamily:"'IBM Plex Mono',monospace"}}>12-month instability trend</div>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:6}}>
          ...
          <div style={{marginLeft:"auto",fontSize:7.5,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace",alignSelf:"center"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"12px 14px",display:"flex",flexDirection:"column",gap:16}}>
        ...
        {ctx?(
          <div style={{padding:12,background:riskBg(country.risk),borderRadius:5,border:`1px solid ${col}22`}}>
            ...
            <div style={{fontSize:10,color:"rgba(255,255,255,0.65)",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.7}}>{ctx.why}</div>
          </div>
        ):(
          <div style={{padding:10,background:"rgba(255,255,255,0.02)",borderRadius:5,border:"1px solid rgba(255,255,255,0.06)"}}>
            <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em",marginBottom:4}}>INTELLIGENCE ASSESSMENT</div>
            <div style={{fontSize:9.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",fontStyle:"italic"}}>Detailed assessment not available for this country. Scores derived from V-Dem, Freedom House, ACLED, RSF and TI indices.</div>
          </div>
        )}

        {/* Active signals */}
        <div>
          <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em",marginBottom:6}}>ACTIVE SIGNALS ({signals.filter(s=>s.level!=="STABLE").length} firing)</div>
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            {signals.map((s,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:"rgba(255,255,255,0.025)",borderRadius:4,borderLeft:`2px solid ${sigColor(s.level)}`}}>
                <span style={{fontSize:12,flexShrink:0}}>{s.icon}</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:9.5,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.7)"}}>{s.label}</div>
                  <div style={{fontSize:7.5,color:"rgba(255,255,255,0.22)",fontFamily:"'IBM Plex Mono',monospace"}}>{s.src}</div>
                </div>
        ...
        <div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em"}}>7-DAY EVENT TIMELINE</div>
            <div style={{fontSize:7.5,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace"}}>ACLED 2024 pre-loaded · est. daily events</div>
          </div>
          <div style={{display:"flex",gap:4,alignItems:"flex-end",height:48}}>
            {daily.map((d,i)=>(
              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                <div style={{fontSize:7,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace"}}>{d.events||"0"}</div>
                ...
                <div style={{fontSize:6.5,color:"rgba(255,255,255,0.25)",fontFamily:"'IBM Plex Mono',monospace",whiteSpace:"nowrap"}}>{d.day}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Pillar bars */}
        <div>
          <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em",marginBottom:6}}>PILLAR BREAKDOWN <span style={{color:"rgba(255,255,255,0.15)"}}>(higher = more risk)</span></div>
          {PILLAR_KEYS.map(k=>(
            <div key={k} style={{marginBottom:7}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:8.5,color:"rgba(255,255,255,0.42)",marginBottom:2,fontFamily:"'IBM Plex Mono',monospace"}}>
                <span>{PILLAR_META[k].icon} {PILLAR_META[k].label} <span style={{color:"rgba(255,255,255,0.18)"}}>({PILLAR_META[k].src})</span></span>
        ...
        {/* Key indices grid */}
        <div>
          <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em",marginBottom:6}}>KEY INDICES</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
            {[
              ...
              <div key={l} style={{padding:"10px 12px",background:"rgba(255,255,255,0.025)",borderRadius:4}}>
                <div style={{fontSize:7.5,color:"rgba(255,255,255,0.28)",fontFamily:"'IBM Plex Mono',monospace"}}>{l}</div>
                {v!=null?(
                  <div style={{fontSize:12,fontWeight:600,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.78)",marginTop:1}}>{v}</div>
                ):(
                  <div style={{fontSize:9,color:"rgba(255,255,255,0.25)",fontFamily:"'IBM Plex Mono',monospace",marginTop:1,fontStyle:"italic"}}>Not available</div>
                )}
                <div style={{fontSize:7,color:"rgba(255,255,255,0.18)",fontFamily:"'IBM Plex Mono',monospace"}}>{s}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {ctx&&(
          <div>
            <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em",marginBottom:6}}>KEY EVENTS (ACLED / CPJ / HRW / UN 2023–24)</div>
            {ctx.events.map((e,i)=>(
              <div key={i} style={{display:"flex",gap:7,marginBottom:6,fontSize:9.5,color:"rgba(255,255,255,0.55)",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.5,paddingBottom:6,borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
        ...
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
            <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em"}}>
            ...
            {updatedAt&&!newsLoading&&(
              <div style={{fontSize:7,color:"rgba(255,255,255,0.18)",fontFamily:"'IBM Plex Mono',monospace"}}>GDELT · {updatedAt.toLocaleTimeString()}</div>
            )}
          </div>

          {newsError==="NO_DATA"&&(
            <div style={{padding:12,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:5,textAlign:"center"}}>
              <div style={{fontSize:9.5,color:"rgba(255,255,255,0.28)",fontFamily:"'IBM Plex Mono',monospace"}}>No news articles found for {country.name} in the past 7 days</div>
              <div style={{fontSize:8,color:"rgba(255,255,255,0.18)",fontFamily:"'IBM Plex Mono',monospace",marginTop:3}}>Source: GDELT Document API</div>
            </div>
          )}
          {newsError==="UNAVAILABLE"&&(
            <div style={{padding:12,background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.18)",borderRadius:6}}>
              ...
              <div style={{fontSize:8,color:"rgba(255,255,255,0.22)",fontFamily:"'IBM Plex Mono',monospace",marginTop:2}}>Live news data cannot be retrieved. Check gdeltproject.org for status.</div>
            </div>
          )}
          {!newsError&&!newsLoading&&articles.length===0&&(
            <div style={{padding:12,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:5,textAlign:"center"}}>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.25)",fontFamily:"'IBM Plex Mono',monospace",fontStyle:"italic"}}>Fetching live news…</div>
            </div>
          )}
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {articles.map((a,i)=>(
              <div key={i} style={{padding:"8px 10px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:4,borderLeft:`2px solid ${col}`,animation:"fadeIn 0.3s ease"}}>
                <a href={a.url} target="_blank" rel="noopener noreferrer"
                  style={{fontSize:10,fontWeight:500,fontFamily:"'IBM Plex Sans',sans-serif",color:"rgba(255,255,255,0.8)",lineHeight:1.4,display:"block",marginBottom:4}}>
                  {a.title}
                </a>
                <div style={{display:"flex",gap:8,fontSize:8,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:16,alignItems:"start"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div style={{...S.card}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",marginBottom:16}}>
            <div style={{display:"flex",flexDirection:"column",gap:3}}>
              <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.06em"}}>COUNTRY</div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:3}}>
              <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.06em"}}>TIMESPAN</div>
            ...
            <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:6,fontSize:8,color:"rgba(255,255,255,0.25)",fontFamily:"'IBM Plex Mono',monospace"}}>
        ...
        <div style={S.card}>
          ...
          {/* Error states */}
          {err==="NO_DATA"&&(
            <div style={{textAlign:"center",padding:"32px 16px",background:"rgba(255,255,255,0.02)",borderRadius:6,border:"1px solid rgba(255,255,255,0.05)"}}>
              <div style={{fontSize:22,marginBottom:8,opacity:0.3}}>◎</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.35)",fontFamily:"'IBM Plex Mono',monospace",marginBottom:4}}>
          ...
          {err==="UNAVAILABLE"&&(
            <div style={{padding:12,background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.18)",borderRadius:6}}>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",fontFamily:"'IBM Plex Mono',monospace"}}>⚠ GDELT API temporarily unavailable</div>
              <div style={{fontSize:8.5,color:"rgba(255,255,255,0.28)",fontFamily:"'IBM Plex Mono',monospace",marginTop:3}}>Live news data cannot be retrieved at this time. Check gdeltproject.org for status.</div>
            </div>
          )}
          {loading&&(
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              {[...Array(5)].map((_,i)=>(
                <div key={i} style={{padding:12,background:"rgba(255,255,255,0.02)",borderRadius:5,border:"1px solid rgba(255,255,255,0.04)"}}>
                  <div style={{height:11,background:"rgba(255,255,255,0.06)",borderRadius:3,marginBottom:6,width:`${70+Math.random()*25}%`}}/>
                  <div style={{height:8,background:"rgba(255,255,255,0.04)",borderRadius:3,width:"40%"}}/>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {results.map((a,i)=>(
              <a key={i} href={a.url} target="_blank" rel="noopener noreferrer"
                style={{display:"block",padding:"10px 12px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:5,borderLeft:`3px solid ${topic.color}`,animation:"fadeIn 0.25s ease",textDecoration:"none",transition:"background 0.15s",cursor:"pointer"}}
                ...
                <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                  <span style={{fontSize:8.5,color:"rgba(255,255,255,0.35)",fontFamily:"'IBM Plex Mono',monospace"}}>{a.domain||a.sourcecountry||"Unknown source"}</span>
                  {a.sourcecountry&&a.domain&&<span style={{fontSize:8,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace"}}>{a.sourcecountry}</span>}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{...S.card,borderTop:`3px solid ${riskColor(countryObj.risk)}`}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div>
                ...
                <div style={{fontSize:8,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace"}}>{countryObj.region}</div>
              </div>
              <div style={{textAlign:"right"}}>
            ...
            {[["V-Dem",countryObj.vdem?.toFixed(2),"V-Dem 2024"],["Freedom House",countryObj.fh+"/100","FH 2024"],["Press Freedom",countryObj.rsf+"/100","RSF 2024"],["ACLED 2024",countryObj.acled_events?.toLocaleString(),"pre-loaded"]].map(([l,v,s])=>(
              <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid rgba(255,255,255,0.04)",fontSize:9,fontFamily:"'IBM Plex Mono',monospace"}}>
                <span style={{color:"rgba(255,255,255,0.35)"}}>{l}</span>
                <span style={{color:"rgba(255,255,255,0.7)"}}>{v} <span style={{color:"rgba(255,255,255,0.2)",fontSize:7.5}}>({s})</span></span>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={S.card}>
          ...
          {[
            ...
            <div key={i} style={{marginBottom:16}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"rgba(255,255,255,0.45)",marginBottom:1,fontFamily:"'IBM Plex Mono',monospace"}}>
                <span>{t.inst}</span><span style={{color:t.trust>50?"#22c55e":t.trust>35?"#f59e0b":"#ef4444"}}>{t.trust}%</span>
              </div>
              <div style={{fontSize:7.5,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace",marginBottom:2}}>({t.src})</div>
        ...
        <div style={{...S.card,border:"1px solid rgba(239,68,68,0.15)"}}>
          ...
          {[
            ...
            <div key={i} style={{marginBottom:16}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"rgba(255,255,255,0.43)",marginBottom:1,fontFamily:"'IBM Plex Mono',monospace"}}>
                <span>{n.l}</span><span style={{color:riskColor(n.v)}}>{n.v}</span>
              </div>
              <div style={{fontSize:7.5,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace",marginBottom:2}}>({n.s})</div>
              <Bar val={n.v}/>
            </div>
          ))}
        </div>
    ...
    <div style={S.root}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        select{background:#111118;border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.65);padding:5px 8px;border-radius:4px;font-family:'IBM Plex Mono',monospace;font-size:10px;outline:none;cursor:pointer;}
        input[type=text]{background:#111118;border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.65);padding:5px 10px;border-radius:4px;font-family:'IBM Plex Mono',monospace;font-size:10px;outline:none;}
      ...
      <header style={S.header}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <img src={process.env.PUBLIC_URL+"/logo.png"} alt="Logo" style={{width:34,height:34,objectFit:"contain",filter:"brightness(0) invert(1)"}}/>
          <div>
            <div style={{fontSize:15,fontWeight:700,letterSpacing:"0.1em",color:"#fff",fontFamily:"'IBM Plex Sans',sans-serif"}}>SIGNAL</div>
            <div style={{fontSize:8.5,color:"rgba(255,255,255,0.3)",letterSpacing:"0.06em",fontFamily:"'IBM Plex Mono',monospace"}}>Democratic Risk Intelligence · {COUNTRIES.length} countries</div>
          </div>
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,fontSize:8.5,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.3)"}}>
      ...
      <div style={{padding:"8px 24px",background:"rgba(255,255,255,0.015)",borderBottom:"1px solid rgba(255,255,255,0.04)",fontSize:9.5,color:"rgba(255,255,255,0.33)",fontFamily:"'IBM Plex Mono',monospace"}}>
          ...
          <div style={{position:"relative"}}>
            ...
            <div style={{background:"#0d0d1a",borderRadius:8,overflow:"hidden",border:"1px solid rgba(255,255,255,0.06)",marginBottom:16}}>
                  ...
                  {[["HIGH ALERT","#ef4444","≥75"],["ELEVATED","#f59e0b","55–74"],["MONITORING","#eab308","35–54"],["STABLE","#22c55e","<35"]].map(([l,c,r])=>(
                    <div key={l} style={{display:"flex",alignItems:"center",gap:7}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:7,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.2)"}}>{r}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{display:"flex",gap:20,padding:"10px 16px",borderTop:"1px solid rgba(255,255,255,0.04)",alignItems:"center"}}>
                {[
                  ...
                  <div key={l} style={{fontSize:8.5,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.35)"}}>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                <thead>
                  <tr>{["Country","Region","Risk","Trend","ACLED 2024","V-Dem","FH"].map(h=>(
                    <th key={h} style={{padding:"5px 10px",textAlign:"left",fontFamily:"'IBM Plex Mono',monospace",fontSize:8,color:"rgba(255,255,255,0.3)",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>{h}</th>
                  ))}</tr>
                </thead>
                <tbody>
                  {topRisks.map(c=>(
                    <tr key={c.id}
                      style={{borderBottom:"1px solid rgba(255,255,255,0.03)",cursor:"pointer",background:sel===c.id?`${riskColor(c.risk)}12`:"transparent",transition:"background 0.15s"}}
                      ...
                      <td style={{padding:"8px 10px",fontSize:8.5,color:"rgba(255,255,255,0.35)",fontFamily:"'IBM Plex Mono',monospace"}}>{c.region}</td>
                      ...
                      <td style={{padding:"8px 10px",fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.45)",fontSize:10}}>{c.acled_events?.toLocaleString()}</td>
                      <td style={{padding:"8px 10px",fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.45)",fontSize:10}}>{c.vdem?.toFixed(2)}</td>
                      <td style={{padding:"8px 10px",fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.45)",fontSize:10}}>{c.fh}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              <div style={S.card}>
                ...
                <div style={{textAlign:"center",padding:"4px 0 10px"}}>
                  ...
                  <div style={{fontSize:8.5,color:"rgba(255,255,255,0.28)",marginTop:3,fontFamily:"'IBM Plex Mono',monospace"}}>P(instability event · 12 months) · CI ±8%</div>
                </div>
                {[["3 months",28],["6 months",41],["12 months",escalP]].map(([l,v])=>(
                  <div key={l} style={{marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"rgba(255,255,255,0.38)",marginBottom:2,fontFamily:"'IBM Plex Mono',monospace"}}><span>{l}</span><span style={{color:"#f59e0b"}}>{v}%</span></div>
                    ...
                    <div key={r} style={{marginBottom:9}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"rgba(255,255,255,0.42)",marginBottom:2,fontFamily:"'IBM Plex Mono',monospace"}}>
                        ...
                        <span style={{color:riskColor(avg)}}>{avg}
                          ...
                          <span style={{color:"rgba(255,255,255,0.2)",marginLeft:6}}>{rc.length} states</span>
                        </span>
                      </div>
                      <Bar val={avg}/>
                    </div>
            ...
            <div style={S.card}>
              <div style={S.ch}>
                ...
                <span style={{...S.cs,marginLeft:"auto"}}>SIGNAL · {new Date().toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}</span>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
                {[
                  ...
                  <div key={i} style={{padding:14,background:"rgba(255,255,255,0.02)",borderRadius:6,border:`1px solid ${b.color}1a`,borderTop:`2px solid ${b.color}`}}>
                    ...
                    <p style={{fontSize:9.5,color:"rgba(255,255,255,0.55)",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.75,marginBottom:8}}>{b.body}</p>
                    <div style={{fontSize:7.5,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace",borderTop:"1px solid rgba(255,255,255,0.05)",paddingTop:6}}>{b.src}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              <div style={S.card}>
                ...
                {[...COUNTRIES].sort((a,b)=>b.trend-a.trend).slice(0,5).map((c,i)=>(
                  <div key={c.id} onClick={()=>setSel(c.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.04)",cursor:"pointer"}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:8,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace"}}>{c.region}</div>
                    </div>
                    <div style={{textAlign:"right"}}>
                ...
                {PILLAR_KEYS.map(k=>{
                  ...
                  return(
                    <div key={k} onClick={()=>{setTab("health");setSelPillar(k);}}
                      style={{padding:14,background:"rgba(255,255,255,0.025)",borderRadius:6,border:`1px solid ${riskColor(avg)}22`,borderTop:`2px solid ${riskColor(avg)}`,cursor:"pointer",transition:"background 0.15s"}}>
                      <div style={{fontSize:18,marginBottom:4}}>{PILLAR_META[k].icon}</div>
                      <div style={{fontSize:9,fontWeight:600,color:"rgba(255,255,255,0.6)",fontFamily:"'IBM Plex Sans',sans-serif",marginBottom:6,lineHeight:1.3}}>{PILLAR_META[k].label}</div>
                      <div style={{fontSize:26,fontWeight:700,color:riskColor(avg),fontFamily:"'IBM Plex Sans',sans-serif",lineHeight:1}}>{avg}</div>
                      <div style={{fontSize:7.5,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace",marginTop:4}}>global avg</div>
                      <div style={{marginTop:8}}><Bar val={avg}/></div>
                      <div style={{marginTop:6,fontSize:7.5,color:"rgba(255,255,255,0.25)",fontFamily:"'IBM Plex Mono',monospace"}}>
                        worst: <span style={{color:riskColor(worst[k])}}>{worst.name} ({worst[k]})</span>
                      </div>
                      <div style={{fontSize:7,color:"rgba(255,255,255,0.15)",fontFamily:"'IBM Plex Mono',monospace",marginTop:2}}>{PILLAR_META[k].src}</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"260px 1fr",gap:16,alignItems:"start"}}>
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div style={S.card}>
                ...
                <div style={{fontSize:9,color:"rgba(255,255,255,0.33)",fontFamily:"'IBM Plex Mono',monospace",marginBottom:16,padding:8,background:"rgba(255,255,255,0.02)",borderRadius:4,lineHeight:1.6}}>
                  ...
                  return(
                    <button key={k} style={{...S.pBtn,...(selPillar===k?S.pBtnA:{})}} onClick={()=>setSelPillar(k)}>
                      ...
                      <div style={{flex:1,textAlign:"left",minWidth:0}}>
                        ...
                        <div style={{fontSize:7.5,color:"rgba(255,255,255,0.28)",fontFamily:"'IBM Plex Mono',monospace",marginTop:1}}>{p.src}</div>
                      </div>
                      <div style={{textAlign:"right",flexShrink:0}}>
                        <div style={{fontSize:17,fontWeight:700,color:riskColor(avg),fontFamily:"'IBM Plex Sans',sans-serif"}}>{avg}</div>
                        <div style={{fontSize:7,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace"}}>avg</div>
                      </div>
                    </button>
                  );
                })}
            ...
            <div style={S.card}>
              {(()=>{
                ...
                return(<>
                  ...
                  <div style={{padding:12,background:"rgba(255,255,255,0.02)",borderRadius:5,border:"1px solid rgba(255,255,255,0.05)",marginBottom:16}}>
                    <p style={{fontSize:10,color:"rgba(255,255,255,0.48)",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.7}}>{p.desc}</p>
                  </div>
                  <div style={{overflowX:"auto",maxHeight:520,overflowY:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                      <thead style={{position:"sticky",top:0,background:"#0a0a0f"}}>
                        <tr>{["Country","Region","Score","12mo","Trend"].map(h=>(
                          <th key={h} style={{padding:"6px 10px",textAlign:"left",fontFamily:"'IBM Plex Mono',monospace",fontSize:8.5,color:"rgba(255,255,255,0.3)",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>{h}</th>
                        ))}</tr>
                      </thead>
                      <tbody>
                        {[...filtered].sort((a,b)=>b[selPillar]-a[selPillar]).map(c=>(
                          <tr key={c.id} style={{borderBottom:"1px solid rgba(255,255,255,0.025)",cursor:"pointer",background:sel===c.id?"rgba(255,255,255,0.04)":"transparent"}}
                            onClick={()=>setSel(c.id)}>
                            <td style={{padding:"7px 10px",fontWeight:600,fontFamily:"'IBM Plex Sans',sans-serif",fontSize:12}}>{c.name}</td>
                            <td style={{padding:"7px 10px",fontSize:8.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace"}}>{c.region}</td>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                    <thead style={{position:"sticky",top:0,background:"#0a0a0f",zIndex:1}}>
                      <tr>{["Country","Region","GDP Risk","Youth %","Conflict","Media","Internet","V-Dem","OVERALL"].map(h=>(
                        <th key={h} style={{padding:"6px 10px",textAlign:"left",fontFamily:"'IBM Plex Mono',monospace",fontSize:8,color:"rgba(255,255,255,0.3)",borderBottom:"1px solid rgba(255,255,255,0.08)",whiteSpace:"nowrap"}}>{h}</th>
                      ))}</tr>
                    </thead>
                    <tbody>
                      {[...filtered].sort((a,b)=>b.risk-a.risk).map(c=>{
                        ...
                        return(
                          <tr key={c.id} style={{borderBottom:"1px solid rgba(255,255,255,0.025)",cursor:"pointer",background:sel===c.id?"rgba(255,255,255,0.04)":"transparent"}}
                            ...
                            <td style={{padding:"7px 10px",fontSize:8,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",whiteSpace:"nowrap"}}>{c.region}</td>
                            {sigs.map((s,si)=>(
                  ...
                  {[...COUNTRIES].filter(c=>{
                    ...
                    <div key={c.id} onClick={()=>setSel(c.id)}
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:7.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace"}}>{c.region}</div>
                      </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div style={S.card}>
                ...
                {[
                  ...
                  <div key={i} style={{marginBottom:16}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:9.5,color:"rgba(255,255,255,0.5)",marginBottom:1,fontFamily:"'IBM Plex Mono',monospace"}}>
                      <span>{i+1}. {d.d}</span><span style={{color:"#f59e0b"}}>{d.w}%</span>
                    </div>
                    <div style={{fontSize:7.5,color:"rgba(255,255,255,0.2)",fontFamily:"'IBM Plex Mono',monospace",marginBottom:2}}>({d.s})</div>
              ...
              <div style={S.card}>
                ...
                {[
                  ...
                  <div key={l} style={{marginBottom:7,paddingBottom:7,borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                    <div style={{fontSize:9,fontWeight:600,color:"rgba(255,255,255,0.5)",fontFamily:"'IBM Plex Mono',monospace"}}>{l}</div>
                    <div style={{fontSize:8,color:"rgba(255,255,255,0.28)",fontFamily:"'IBM Plex Mono',monospace",marginTop:1,lineHeight:1.5}}>{d}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              {SCENARIOS.map(s=>(
                <button key={s.id} style={{...S.pBtn,...(selScenario===s.id?S.pBtnA:{})}} onClick={()=>setSelScenario(s.id)}>
                  ...
                  <div style={{flex:1,textAlign:"left"}}>
                    ...
                    <div style={{fontSize:8,color:"rgba(255,255,255,0.28)",fontFamily:"'IBM Plex Mono',monospace",marginTop:1}}>P: {s.prob}% · Recovery: {s.recovery}%</div>
                  </div>
                </button>
              ))}
            </div>
            ...
            {scenario&&(
              <div style={{display:"flex",flexDirection:"column",gap:16}}>
                <div style={S.card}>
                  ...
                  <div style={{padding:12,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:5,marginBottom:16}}>
                    <div style={S.sl}>SUMMARY</div>
                    <p style={{fontSize:10.5,color:"rgba(255,255,255,0.58)",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.8,marginTop:4}}>{scenario.summary}</p>
                  </div>
                  <div style={{padding:12,background:"rgba(29,78,216,0.06)",border:"1px solid rgba(29,78,216,0.2)",borderRadius:5,marginBottom:16}}>
                    ...
                    <p style={{fontSize:9,color:"rgba(255,255,255,0.42)",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.7,marginTop:3}}>{scenario.method}</p>
                  </div>
                  ...
                  {scenario.drivers.map((d,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                      ...
                      <span style={{fontSize:10.5,color:"rgba(255,255,255,0.58)",fontFamily:"'IBM Plex Mono',monospace"}}>{d}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={S.card}>
              <div style={S.ch}>
                ...
                <div style={{display:"flex",gap:8,marginLeft:"auto",alignItems:"center"}}>
                  ...
                  <span style={{color:"rgba(255,255,255,0.2)",fontSize:10,fontFamily:"'IBM Plex Mono',monospace"}}>vs</span>
                ...
                <div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 80px 1fr",marginBottom:16}}>
                    {[countryA,countryB].map((c,idx)=>(
                      <div key={c.id} style={{textAlign:"center",padding:12,background:"rgba(255,255,255,0.02)",borderRadius:idx===0?"6px 0 0 6px":"0 6px 6px 0",border:"1px solid rgba(255,255,255,0.06)",borderRight:idx===0?"none":"",borderLeft:idx===1?"none":""}}>
                        <div style={{fontSize:16,fontWeight:700,fontFamily:"'IBM Plex Sans',sans-serif"}}>{c.name}</div>
                        <div style={{fontSize:8.5,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace",marginBottom:6}}>{c.region}</div>
                  ...
                  {PILLAR_KEYS.map(k=>(
                    <div key={k} style={{display:"grid",gridTemplateColumns:"1fr 130px 1fr",gap:16,marginBottom:9,alignItems:"center"}}>
                      ...
                      <div style={{textAlign:"center",fontSize:8.5,color:"rgba(255,255,255,0.35)",fontFamily:"'IBM Plex Mono',monospace"}}>{PILLAR_META[k].icon} {PILLAR_META[k].label}</div>
                      <div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginTop:12}}>
                    {[["V-Dem",countryA.vdem?.toFixed(2),countryB.vdem?.toFixed(2),"V-Dem 2024"],["Freedom House",countryA.fh,countryB.fh,"FH 2024"],["Press Freedom",countryA.rsf,countryB.rsf,"RSF 2024"],["TI Score",countryA.ti,countryB.ti,"TI 2024"],["ACLED 2024",countryA.acled_events?.toLocaleString(),countryB.acled_events?.toLocaleString(),"ACLED"],["Youth Unemp.",countryA.youth_unemp+"%",countryB.youth_unemp+"%","ILO 2023"]].map(([l,a,b,s])=>(
                      <div key={l} style={{padding:12,background:"rgba(255,255,255,0.02)",borderRadius:5}}>
                        <div style={{fontSize:8,color:"rgba(255,255,255,0.25)",fontFamily:"'IBM Plex Mono',monospace",marginBottom:3}}>{l} <span style={{fontSize:7,color:"rgba(255,255,255,0.14)"}}>({s})</span></div>
                        <div style={{display:"flex",justifyContent:"space-between"}}>
                          <span style={{fontSize:13,fontWeight:700,color:"rgba(255,255,255,0.7)",fontFamily:"'IBM Plex Mono',monospace"}}>{a}</span>
                          <span style={{fontSize:13,fontWeight:700,color:"rgba(255,255,255,0.7)",fontFamily:"'IBM Plex Mono',monospace"}}>{b}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                  <thead>
                    <tr>{["Country","Risk","Electoral","Civic","Rule of Law","Violence","Digital","Trend","ACLED 2024"].map(h=>(
                      <th key={h} style={{padding:"6px 10px",textAlign:"left",fontFamily:"'IBM Plex Mono',monospace",fontSize:8.5,color:"rgba(255,255,255,0.3)",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>{h}</th>
                    ))}</tr>
                  </thead>
                  <tbody>
                  ...
                  <div key={c.id} onClick={()=>setSel(c.id)} style={{padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.04)",cursor:"pointer"}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,paddingLeft:18}}>
                      {[
                        ...
                        <div key={a.label} style={{padding:"4px 6px",background:"rgba(255,255,255,0.02)",borderRadius:3,borderLeft:`2px solid ${a.col}`}}>
                          <div style={{fontSize:7,color:"rgba(255,255,255,0.3)",fontFamily:"'IBM Plex Mono',monospace"}}>{a.label}</div>
                ...
                <div style={S.card}>
                  ...
                  {[
                    ...
                    <div key={i} style={{padding:"9px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                      ...
                      <div style={{fontSize:9,color:"rgba(255,255,255,0.45)",fontFamily:"'IBM Plex Mono',monospace",marginBottom:4,lineHeight:1.4}}>{l.lever}</div>
              ...
              <div style={S.card}>
                ...
                {[
                  ...
                  <div key={i} style={{padding:"9px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                    ...
                    <p style={{fontSize:9,color:"rgba(255,255,255,0.45)",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.7}}>{s.desc}</p>
                  </div>
                ))}
              </div>
...
const S = {
  root:{background:"#0a0a0f",minHeight:"100vh",color:"#fff",fontFamily:"'IBM Plex Sans',sans-serif",fontSize:13},
  header:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 24px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:"#0a0a0f",position:"sticky",top:0,zIndex:100,flexWrap:"wrap",gap:10},
  chip:{display:"flex",alignItems:"center",gap:5,padding:"4px 11px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:20,fontSize:9.5,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.5)"},
  nav:{display:"flex",overflowX:"auto",borderBottom:"1px solid rgba(255,255,255,0.06)",padding:"0 24px",gap:2,background:"#0a0a0f"},
  navBtn:{display:"flex",alignItems:"center",padding:"11px 14px",background:"transparent",border:"none",color:"rgba(255,255,255,0.38)",cursor:"pointer",fontSize:10,fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.04em",whiteSpace:"nowrap",borderBottom:"2px solid transparent",transition:"all 0.15s"},
  navA:{color:"#f59e0b",borderBottom:"2px solid #f59e0b"},
  card:{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,padding:16,overflow:"hidden"},
  ch:{display:"flex",alignItems:"center",gap:6,marginBottom:16,flexWrap:"wrap"},
  ct:{fontSize:9.5,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.45)",letterSpacing:"0.08em",fontWeight:500},
  cs:{fontSize:8,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.2)"},
  badge:{padding:"2px 7px",borderRadius:4,fontSize:8,fontFamily:"'IBM Plex Mono',monospace"},
  statBox:{padding:14,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:6},
  sl:{fontSize:8,fontFamily:"'IBM Plex Mono',monospace",color:"rgba(255,255,255,0.27)",letterSpacing:"0.1em",marginBottom:6,textTransform:"uppercase"},
  pBtn:{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"10px 12px",background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:6,color:"rgba(255,255,255,0.6)",cursor:"pointer",textAlign:"left",marginBottom:6,transition:"all 0.15s"},
  pBtnA:{background:"rgba(245,158,11,0.07)",border:"1px solid rgba(245,158,11,0.22)",color:"#f59e0b"},
  mapBtn:{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",color:"rgba(255,255,255,0.5)",cursor:"pointer",borderRadius:4,padding:"4px 9px",fontSize:11,fontFamily:"'IBM Plex Mono',monospace",transition:"all 0.15s"},
  exportBtn:{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",color:"rgba(255,255,255,0.5)",cursor:"pointer",borderRadius:4,padding:"4px 10px",fontSize:8.5,fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.05em",transition:"all 0.15s"},
};